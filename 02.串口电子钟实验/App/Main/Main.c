...
static  void  Proc1SecTask(void)
{
 ...
 // i16 min;
  ...
  if(Get1SecFlag())    //判断1s标志状态
  {
    ...
    year = GetTimeVal(TIME_VAL_YEAR);
    month = GetTimeVal(TIME_VAL_MONTH);
    day = GetTimeVal(TIME_VAL_DAY);
    week = GetTimeVal(TIME_VAL_WEEK);
    
        
    //DispTime(hour, min, sec);
     DispTime(year,  month,  day,  week,  sec);
...
{ 
 ...
 /*SetTimeVal(TIME_VAL_HOUR, 23);
  ...
  SetTimeVal(TIME_VAL_SEC, 50);
  */
  SetTimeVal(TIME_VAL_YEAR, 2024);
  SetTimeVal(TIME_VAL_MONTH, 5);
  SetTimeVal(TIME_VAL_DAY, 20);
  SetTimeVal(TIME_VAL_WEEK, 1);
